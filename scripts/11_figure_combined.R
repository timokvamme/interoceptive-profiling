...
# Borders follow the consensus ranking of 09_item_selection_analysis.R, the
# same item sets used everywhere else in the paper.
consensus_items <- consensus_df$item
top_21_items <- head(consensus_items, 21)
top_10_items <- head(consensus_items, 10)
...
# Community labels are built from the detected membership, not hard-coded.
comm_sizes <- table(layout_df$community)
community_names <- sapply(names(comm_sizes), function(k) {
  members <- layout_df$item[layout_df$community == k]
  n_ias <- sum(grepl("^ias_", members)); n_iats <- sum(grepl("^iats_", members))
  paste0("Community ", k, " (", n_ias, " IAS, ", n_iats, " IATS)")
})

ias19_pos <- layout_df[layout_df$item == "ias_19", ]
y_range <- diff(range(layout_df$y))

n_ias_10 <- sum(grepl("^ias_", top_10_items))
n_iats_10 <- sum(grepl("^iats_", top_10_items))
...
ggsave(file.path(plots_dir, "figure_3_minimal_subtyping_network.png"), final_figure,
       width = 16, height = 18, dpi = 300)

cat("Combined figure saved: figure_3_minimal_subtyping_network.png\n")
...
cat("\nCombined figure:\n")
cat("  - figure_3_minimal_subtyping_network.png (4-panel publication figure)\n")
