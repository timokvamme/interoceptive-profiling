...
# working repo, and in scripts/ in the public release.
for _c in (script_dir, os.path.dirname(script_dir)):
    if os.path.exists(os.path.join(_c, "dfc_interoception_profiling.csv")):
        script_dir = _c
        break
os.chdir(script_dir)

PLOTS_DIR = os.path.join(script_dir, "plots", "supplementary_plots")
DATA_OUT = os.path.join(script_dir, "supplementary_data")
ANALYSIS_OUT = os.path.join(script_dir, "analysis_output")
...
class _Tee:
    """Send everything printed to both the console and 16_output.txt.

    Without this the output file was written once by hand and then never
    refreshed, so it kept reporting a sample size the analysis had moved past.
    """
...
_report = open(os.path.join(ANALYSIS_OUT, "16_output.txt"), "w", encoding="utf-8")
...
# Load data
# ---------------------------------------------------------------------------
df = pd.read_csv("dfc_interoception_profiling.csv")
labels_df = pd.read_csv("supplementary_data/data_with_k_means_clusters.csv")
