...
# Plot the point estimates and percentile bootstrap CIs computed by
# 19_ert_exact_statistics.R (run 19 first), so that the figure shows the same
# quantities as the main text; the bootstrap means printed above are shown in
# the text of the caption only.
dom_csv <- file.path("supplementary_data", "pathway_dominance_by_profile.csv")
...
plot_data <- data.frame(
  Profile = factor(dom_src$profile, levels = c("Hypervigilant", "Uncertain", "Efficient")),
  TAS = dom_src$pct_alex, TAS_low = dom_src$lo_alex, TAS_high = dom_src$hi_alex,
  Somatic = dom_src$pct_som, Som_low = dom_src$lo_som, Som_high = dom_src$hi_som
)
...
plot_long$ci_low <- ifelse(plot_long$Pathway == "TAS", plot_long$TAS_low, plot_long$Som_low)
plot_long$ci_high <- ifelse(plot_long$Pathway == "TAS", plot_long$TAS_high, plot_long$Som_high)
...
p_dominance <- ggplot(plot_long, aes(x = Profile, y = Percentage, fill = Pathway)) +
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1), limits = c(0, 108), breaks = seq(0, 100, 25)) +
  labs(
    title = "Pathway dominance by profile",
    subtitle = "Conditional indirect effect of IAS on MH through each pathway (point estimates, 95% percentile bootstrap CIs)",
    x = NULL,
    y = "Percentage of Total Indirect Effect",
    caption = sprintf("N = %d; 2,000 bootstrap resamples (19_ert_exact_statistics.R)", nrow(dfc))
  ) +
  ...
  geom_text(aes(label = sprintf("%.1f%%", Percentage), y = ci_high + 3),
            position = position_dodge(width = 0.8), size = 4, fontface = "bold")
