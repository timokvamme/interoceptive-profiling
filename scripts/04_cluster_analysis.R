...
#   - figure_2_clustering_profiles.png (publication figure with heatmap + elbow + profiles)
...
create_bar_plot <- function(var, var_label, panel_letter, data, color_map) {
  ...
  # group mean marked instead, which the editorial table names as acceptable
  # and which reads more clearly than a violin at this sample size.
  plot_data <- data %>%
    mutate(.yval = get(var)) %>%
    filter(!is.na(.yval))
  pvals <- get_tukey_pvals(var, data)
  base_max <- max(plot_data$.yval, na.rm = TRUE)
  y_floor <- min(0, min(plot_data$.yval, na.rm = TRUE))
  y_max <- base_max * 1.18
  y_range <- y_max - y_floor

  p <- ggplot(plot_data, aes(x = cluster_short, y = .yval, fill = cluster_short)) +
    geom_boxplot(width = 0.6, alpha = 0.8, color = "black", linewidth = 0.35,
                 outlier.size = 0.7, outlier.alpha = 0.35, outlier.color = "grey30") +
    ...
    coord_cartesian(ylim = c(y_floor, y_max))
...
p_sss8 <- create_bar_plot("sss8", "Somatic Symptoms (SSS-8)", "D", dfc, color_map_short)
p_pcs <- create_bar_plot("pcs", "Pain Catastrophizing (PCS)", "E", dfc, color_map_short)
...
heatmap_img <- png::readPNG(heatmap_path, native = TRUE)
heatmap_panel <- grobTree(
  ...
  textGrob(sprintf("A. Heatmap of variables (N = %d)", nrow(dfc)), x = unit(0.02, "npc"), y = unit(0.98, "npc"),
...
ggsave(file.path(plots_dir, "figure_2_clustering_profiles.png"),
       combined_figure, width = 16, height = 16, dpi = 300)
cat("Publication figure saved: figure_2_clustering_profiles.png\n")
...
age_base_max <- max(demog_df$age, na.rm = TRUE)
age_y_max <- age_base_max * 1.18
age_y_range <- age_y_max - 0

p_age <- ggplot(demog_df, aes(x = cluster_short, y = age, fill = cluster_short)) +
  geom_boxplot(width = 0.6, alpha = 0.8, color = "black", linewidth = 0.35,
               outlier.size = 0.6, outlier.alpha = 0.35, outlier.color = "grey30") +
...
cat("  plots/figure_2_clustering_profiles.png, supplementary_plots/1_s_1_k_selection_methods.png, supplementary_plots/1_s_3_cluster_robustness.png\n")
