...
# The minimal battery is the top 21 items of the consensus ranking written by
# 09_item_selection_analysis.R (item_consensus_ranking.csv is sorted by that
# ranking), so this script and 09 always agree on the item set.
consensus_items <- read.csv(file.path(supplementary_data_dir, "item_consensus_ranking.csv"))$item[1:21]
ias_minimal_items <- consensus_items[grepl("^ias_", consensus_items)]
iats_minimal_items <- consensus_items[grepl("^iats_", consensus_items)]
...
p_coef <- ggplot() +
  ...
  xlim(0, 7) + ylim(0, 10) +
  # Title
  annotate("text", x = 0.2, y = 9.7, label = sprintf("A. Path Coefficients (N = %d)", nrow(dfc)),
...
p_sem_standalone <- ggplot() +
  ...
  annotate("text", x = 6, y = 9.15,
           label = sprintf("Red = worsens MH | Green = improves MH | Dotted = direct | N = %d", nrow(dfc)),
           size = 6, color = "gray40") +
...
# Pathway dominance by profile: point estimates and percentile bootstrap CIs
# computed by 19_ert_exact_statistics.R (run 19 before this script).
dom_csv <- file.path(supplementary_data_dir, "pathway_dominance_by_profile.csv")
if (!file.exists(dom_csv)) stop("Run 19_ert_exact_statistics.R first: ", dom_csv, " is missing")
dom_src <- read.csv(dom_csv)
dom_src <- dom_src[match(c("Hypervigilant", "Uncertain", "Efficient"), dom_src$profile), ]
pathway_dom <- data.frame(
  Profile = factor(c("Hypervigilant", "Uncertain", "Efficient"),
                   levels = c("Hypervigilant", "Uncertain", "Efficient")),
  TAS_pct = dom_src$pct_alex,
  TAS_ci_low = dom_src$lo_alex,
  TAS_ci_high = dom_src$hi_alex,
  Som_pct = dom_src$pct_som,
  Som_ci_low = dom_src$lo_som,
  Som_ci_high = dom_src$hi_som
)
...
# Simple slopes of IAS at each profile's mean attention level, computed by
# 19_ert_exact_statistics.R (OLS on standardized variables, IATS re-centred at
# the profile mean). Each line is drawn through the profile's mean point.
slope_csv <- file.path(supplementary_data_dir, "figure1_simple_slopes_by_profile.csv")
if (!file.exists(slope_csv)) stop("Run 19_ert_exact_statistics.R first: ", slope_csv, " is missing")
slope_src <- read.csv(slope_csv)
km_lab <- read.csv(file.path(supplementary_data_dir, "data_with_k_means_clusters.csv"))
stopifnot(nrow(km_lab) == nrow(dfc), all(km_lab$ias == dfc$ias))
prof_map <- c("Average Accuracy / High Attention" = "Hypervigilant",
              "Low Accuracy / Average Attention" = "Uncertain",
              "High Accuracy / Low Attention" = "Efficient")
prof_of_row <- unname(prof_map[km_lab$cluster_label])
z_ias <- as.numeric(scale(dfc$ias))
z_som <- as.numeric(scale(dfc$somatic_combined))
z_tas <- as.numeric(scale(dfc$tas))
slope_frame <- function(outcome, yz) {
  d <- slope_src[slope_src$outcome == outcome, ]
  d <- d[match(c("Hypervigilant", "Uncertain", "Efficient"), d$profile), ]
  mx <- tapply(z_ias, prof_of_row, mean)[d$profile]
  my <- tapply(yz, prof_of_row, mean)[d$profile]
  stars <- ifelse(d$p < 0.001, "***", ifelse(d$p < 0.01, "**", ifelse(d$p < 0.05, "*", " (n.s.)")))
  data.frame(profile = d$profile, slope = d$slope, intercept = my - d$slope * mx,
             label = sprintf("\u03b2 = %.3f%s", d$slope, stars))
}
somatic_slopes <- slope_frame("somatic", z_som)
tas_slopes <- slope_frame("tas", z_tas)
...
p_somatic <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Effect of IAS on somatic distress varies by profile",
  ...
  annotate("label", x = -1.6, y = 0.95, label = somatic_slopes$label[1], color = "#F59E0B", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
  annotate("label", x = -1.6, y = 0.25, label = somatic_slopes$label[2], color = "#EF4444", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
  annotate("label", x = -1.6, y = -0.45, label = somatic_slopes$label[3], color = "#22C55E", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
...
p_tas <- ggplot() +
  ...
  # Add slope annotations - labels with white background for visibility
  annotate("label", x = -1.6, y = 1.0, label = tas_slopes$label[1], color = "#F59E0B", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
  annotate("label", x = -1.6, y = 0.75, label = tas_slopes$label[2], color = "#EF4444", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
  annotate("label", x = -1.6, y = 0.40, label = tas_slopes$label[3], color = "#22C55E", size = 6.5, hjust = 0, fontface = "bold", fill = "white", label.size = 0.3) +
  coord_cartesian(clip = "off")
...
ggsave(file.path(plots_dir, "figure_1_dual_pathway_to_mh.png"), combined_full,
       width = 18, height = 16, dpi = 300, bg = "white")

cat("Combined figure saved: plots/figure_1_dual_pathway_to_mh.png\n")
