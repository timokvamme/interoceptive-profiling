...
# Resolve the repository root, which is where the data, plots and
# supplementary_data folders live. Two layouts are supported: the working repo,
# where this script sits beside the data file, and the public release, where it
# sits in scripts/.
repo_root = script_dir
for candidate in (script_dir, os.path.dirname(script_dir)):
    if os.path.exists(os.path.join(candidate, "dfc_interoception_profiling.csv")):
        repo_root = candidate
        break
os.chdir(repo_root)

PLOTS_DIR = os.path.join(repo_root, "plots", "supplementary_plots")
DATA_OUT = os.path.join(repo_root, "supplementary_data")
